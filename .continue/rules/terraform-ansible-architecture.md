...
- opentofu code is organized in the `opentofu` directory, with separate subdirectories for different environments (e.g. `trex`, `nanosarus`, and `pterodactyl`). Each host has a different function based on the containers on the host.
