...
- [`opentofu/trex/`](opentofu/trex/) — primary server: Traefik, media (Sonarr/Radarr/Lidarr/Prowlarr/Jellyfin/Seerr/LazyLibrarian), Transmission-over-VPN, FlareSolverr, ArchiveTeam Warrior, Immich, monitoring (Beszel/Dozzle), AI (Ollama/Open WebUI). Jellyfin uses NVIDIA hardware transcoding via the CDI device spec generated by the `nvidia` Ansible role.
...
Three roles applied via [`ansible/site.yml`](ansible/site.yml), each scoped to its own inventory group and tagged so it can be run independently.
...
- `nvidia` (group `gpu`) — installs the NVIDIA driver + prebuilt kernel modules (or DKMS), blacklists `nouveau`, adds the NVIDIA Container Toolkit apt repo, and generates the CDI spec at `/etc/cdi/nvidia.yaml` so Docker containers can request the GPU via `nvidia.com/gpu=all`. If the kernel module is not yet loaded after install (first run on a fresh host), the role fails with a reboot-and-rerun message rather than silently skipping the CDI generation.
...
├── roles/nvidia/     # NVIDIA driver + Container Toolkit + CDI spec
└── site.yml          # applies all three roles, tag-selectable
scripts/
...
ansible-playbook site.yml --tags nvidia   # or --tags gpu
```

The `nvidia` role requires a reboot on first install before the kernel module loads; the role detects this and fails with a reboot-and-rerun message rather than producing a half-configured host.
...
| `nvidia_driver_branch` | `590` | NVIDIA driver branch installed on `gpu` hosts |
| `nvidia_use_dkms` | `false` | Use DKMS modules instead of the prebuilt HWE kernel module package |
| `nvidia_cdi_spec_path` | `/etc/cdi/nvidia.yaml` | Output path for `nvidia-ctk cdi generate` |
